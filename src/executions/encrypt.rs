...
use crate::{
    actions::{self, encrypt_from_threadpool},
...
pub fn encrypt(
    ...
    for input_path in input_paths {
        let output_path = actions::get_output_dir_path(
            &input_path,
            output_path.as_ref(),
            actions::Command::Encrypt,
        );

        actions::overwrite_when_asked(&input_path, overwrite, actions::Command::Encrypt)?;
        ...
        if input_path.is_dir() {
            ...
            rx.iter().take(tries_nb).for_each(|(path, success)| {
                if success {
                    success_paths.push(path.to_owned());
                } else {
                    failed_paths.push(path.to_owned());
                }
            })
