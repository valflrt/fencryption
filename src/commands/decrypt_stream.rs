...
pub fn action(args: &Command) {
    ...
    let mut source = File::open("_test.enc.bin").expect("Failed to read source file");
    let mut dest = File::create("_test.dec.bin").expect("Failed to create destination file");
